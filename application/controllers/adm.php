...
class Adm extends MY_Controller
{
    ...
    public function m_siswa()
    {
        ...
        if ($uri3 == "det") {
            ...
            if ($p->id != 0) {
                ...
                $this->db->query("UPDATE m_siswa SET
								nama = '" . bersih($p, "nama") . "',
								tanggal_daftar = '" . bersih($p, "tanggal_daftar") . "',
								program = '" . bersih($p, "program") . "',
								sekolah = '" . bersih($p, "sekolah") . "',
								no_hp_siswa = '" . bersih($p, "no_hp_siswa") . "',
								nama_ayah = '" . bersih($p, "nama_ayah") . "',
								no_hp_ayah = '" . bersih($p, "no_hp_ayah") . "',
								nama_ibu = '" . bersih($p, "nama_ibu") . "',
								no_hp_ibu = '" . bersih($p, "no_hp_ibu") . "',
								alamat = '" . bersih($p, "alamat") . "',
								keterangan = '" . bersih($p, "keterangan") . "'
								WHERE id = '" . bersih($p, "id") . "'");
                $ket = "edit";
            } else {
                $ket = "tambah";
                $this->db->query("INSERT INTO m_siswa
						VALUES (null,
								'" . bersih($p, "nama") . "',
								'" . bersih($p, "tanggal_daftar") . "',
								'" . bersih($p, "program") . "',
								'" . bersih($p, "sekolah") . "',
								'" . bersih($p, "no_hp_siswa") . "',
								'" . bersih($p, "nama_ayah") . "',
								'" . bersih($p, "no_hp_ayah") . "',
								'" . bersih($p, "nama_ibu") . "',
								'" . bersih($p, "no_hp_ibu") . "',
            ...
            if (!empty($det_user)) {
                $this->db->query("INSERT INTO m_admin VALUES
								(null, '" . bersih($p, 'no_hp_siswa') . $det_user->no_hp_siswa . "',
							md5('admin'), 'siswa', '" . $det_user->id . "')");
                $ret_arr['status']  = "ok";
        ...
        if ($uri3 == "det") {
            ...
            if ($status != "error") {
                ...
                if (!$this->upload->do_upload($file_element_name)) {
                    ...
                    if ($_POST['id'] != "") {
                        $this->db->query("UPDATE m_soal SET
										...
										opsi_d = '" . $_POST['opsi_d'] . "',
										opsi_e = '" . $_POST['opsi_e'] . "',
										jawaban = '" . $_POST['jawaban'] . "'" . $pembuat_soal_u . "
										WHERE id = '" . $_POST['id'] . "'");
                    } else {
    ...
    public function h_ujian()
    {
        ...
        if ($uri3 == "det") {
            ...
            $a['hasil'] = $this->db->query("SELECT m_siswa.nama,
				tr_ikut_ujian.nilai,
				 tr_ikut_ujian.jml_benar,
				  tr_ikut_ujian.nilai_bobot
											FROM tr_ikut_ujian
											INNER JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id
											WHERE tr_ikut_ujian.id_tes = '$uri4'")->result();
            $a['p'] = "guru/m_guru_tes_hasil_detil";
            //echo $this->db->last_query();
        } else {
            $a['p'] = "guru/m_guru_tes_hasil";
        }
    ...
    public function hasil_ujian_cetak()
    {
        ...
        $this->load->view("guru/m_guru_tes_hasil_detil_cetak", $a);
    }
    ...
    public function ikut_ujian()
    {
        ...
        if ($uri3 == "simpan_satu") {
            ...
            //echo var_dump($a);
            $this->load->view('ujian/aaa_ikut_ujian', $a);
        }
    }
    ...
    {
        $this->load->view('login/aaa_login');
    }
...
//administrasi
    ...
    public function m_spp_cetak()
    {
        ...
        if (isset($arg[0])) {
            ...
            // join ke tabel siswa mas
            // ini belum bisa di join karena belum punya kunci



            // logikanya gini

            // di tabel m_spp harus punya foreign key dari tabel m_siswa (id_siswa)
            // dan di tabel m_siswa harus ada Primary Key (id_siswa) << ini sudah ada


            //jadi d tabel siswa harus tambah field baru ta mas n jadikan fk a

// yup, 
            // caranya??
            // nim ini pindah ke tabel m_siswa
            //kalau berdasarkan id siswa mas, jadi si nim d rubah ke id yup
            // paham berarti
            //siap

            // jadi strukturnya juga perlu dikoreksi
// lanjutkan
    ...
    ///////////////////////////////////////////////////
    ///////////////////////////////////////////////////
    public function m_spp_detail_ajax($id_spp = null)
    {

        if ($this->input->is_ajax_request()) {
            if ($this->input->get('id_spp')) {
                $id_spp = $this->input->get('id_spp');
            }
            if (isset($id_spp)) {
                $this->db->where('id_spp', $id_spp);
                $spp_detail        = $this->db->get('m_spp')->result();
                $res['spp_detail'] = $spp_detail;
...
///////////////
        ...
        if ($this->input->post()) {
            ...
            //opn($param);
            if (isset($param['id_spp']) && !empty($param['id_spp'])) {
                $this->db->where('id_spp', $param['id_spp']);
                $this->db->update('m_spp', $param);
            } else {
                $this->db->insert('m_spp', $param);
            }
            redirect(base . '/adm/m_spp', 'refresh');
...
//untuk foto nya mas
    ...
    public function m_upload()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(base . '/adm/m_upload', 'refresh');
        } else {
    ...
    public function delete($id)
    {
        if (isset($id)) {
            ...
            redirect(base . '/adm/m_upload', 'refresh'); //bukannya yang ini ta mas redirectnya
        }
    }
// unduh soal dengan siswa
